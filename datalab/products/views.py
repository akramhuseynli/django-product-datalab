...
def product_upload(request):
    ctx: dict[str, object]={"form":UploadForm()}
...
def product_export(request):
    ...
    path=utils.df_to_excel_response(df,"products_exports.xlsx")
...
def stats_view(request):
    ...
    ctx=dict(monthly=monthly,quarterly=quarterly,by_cat=by_cat,top_sku=top_sku,low_stock=low_stock)
...
def download_template(request):
    ...
    fpath = utils.df_to_excel_response(df,"product_template.xlsx")

    return FileResponse(open(fpath,"rb"),as_attachment=True,filename="product_template.xlsx")